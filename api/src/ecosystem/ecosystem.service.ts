...
const configs = {
  ...
  recyclingPartners: { collection: COLLECTIONS.recyclingPartners, resourceType: "recyclingPartners", defaultStatus: "ACTIVE", defaultPriority: "MEDIUM" },
...
const searchableCollections = [
  ...
  { label: "Recycling partner", href: "/admin/recycling", collection: COLLECTIONS.recyclingPartners },
...
type RecyclingRecord = Record<string, unknown> & { id: string };
type SustainabilityReportRecord = Record<string, unknown> & { id: string };
type SuccessStoryRecord = Record<string, unknown> & { id: string };
type SupportTicketRecord = Record<string, unknown> & { id: string };
type TrainingCohortRecord = Record<string, unknown> & { id: string };
...
type RecyclingStatus = (typeof recyclingStatuses)[number];
...
type TrainingProgrammeType = (typeof trainingProgrammeTypes)[number];
...
export class EcosystemService {
  ...
  async uploadRepairAttachment(id: string, file: UploadedAdminFile | undefined, request: AuthenticatedRequest) {
  ...
  async listRecyclingOperations() {
    const [records, partners] = await Promise.all([
      this.safeList<RecyclingRecord>(COLLECTIONS.recycling, 500),
      this.safeList<Record<string, unknown> & { id: string }>(COLLECTIONS.recyclingPartners, 200)
    ]);
  ...
  async createRecyclingRecord(dto: Record<string, unknown>, request: AuthenticatedRequest) {
    ...
    const estimatedCo2KgAvoided =
      this.safeNumber(dto.estimatedCo2KgAvoided ?? dto.estimatedCo2SavedKg) ?? Math.round(deviceCount * 50);
  ...
  async updateRecyclingRecord(id: string, dto: Record<string, unknown>, request: AuthenticatedRequest) {
    const before = await this.repository.findById<RecyclingRecord>(COLLECTIONS.recycling, id);
    ...
    const after = await this.repository.update<RecyclingRecord>(COLLECTIONS.recycling, id, sanitizePayload(updatePayload));
    await this.log(request, "UPDATE_RECYCLING", "recycling", id, before, after);
    return after;
  }

  async uploadRecyclingAttachment(
    id: string,
    file: UploadedAdminFile | undefined,
    dto: Record<string, unknown>,
    request: AuthenticatedRequest
  ) {
    if (!file?.buffer?.length) {
      throw new BadRequestException("A recycling evidence file is required.");
    }
    ...
    const after = await this.repository.update<RecyclingRecord>(COLLECTIONS.recycling, id, sanitizePayload({
      attachments: [...this.asArray(before.attachments), attachment],
      timeline: [
        ...this.asArray(before.timeline),
  ...
  async recommendRecyclingRoute(id: string, request: AuthenticatedRequest) {
    const before = await this.repository.findById<RecyclingRecord>(COLLECTIONS.recycling, id);
  ...
  async generateRecyclingReportPack(id: string, dto: Record<string, unknown>, request: AuthenticatedRequest) {
    const before = await this.repository.findById<RecyclingRecord>(COLLECTIONS.recycling, id);
    ...
    const reportPack = sanitizePayload({
      id: randomBytes(8).toString("hex"),
  ...
  async listPublishedStories() {
    const stories = await this.repository.list<SuccessStoryRecord>(COLLECTIONS.successStories, 100);
    return stories
  ...
  async listSuccessStoryOperations() {
    const stories = (await this.safeList<SuccessStoryRecord>(COLLECTIONS.successStories, 500))
      .map((story) => this.enrichSuccessStory(story));
    return {
      stories,
  ...
  async createSuccessStoryRecord(dto: Record<string, unknown>, request: AuthenticatedRequest) {
    ...
    return this.enrichSuccessStory(story as SuccessStoryRecord);
  }

  async updateSuccessStoryRecord(id: string, dto: Record<string, unknown>, request: AuthenticatedRequest) {
    const before = await this.repository.findById<SuccessStoryRecord>(COLLECTIONS.successStories, id);
    ...
    const after = await this.repository.update<SuccessStoryRecord>(COLLECTIONS.successStories, id, payload);
    await this.log(request, "UPDATE_SUCCESSSTORIES", "successStories", id, before, after);
    return this.enrichSuccessStory(after);
  }

  async deleteSuccessStory(id: string, request: AuthenticatedRequest) {
    const before = await this.repository.findById<SuccessStoryRecord>(COLLECTIONS.successStories, id);
    await this.repository.delete(COLLECTIONS.successStories, id);
    await this.log(request, "DELETE_SUCCESSSTORIES", "successStories", id, before, null);
    return { id, deleted: true };
  }

  async featureStory(id: string, featured: boolean, request: AuthenticatedRequest) {
    const before = await this.repository.findById<SuccessStoryRecord>(COLLECTIONS.successStories, id);
    const timeline = [
      ...this.asArray(before.timeline),
      this.timelineEntry(featured ? "featured" : "unfeatured", featured ? "Story featured" : "Story removed from featured set", request)
    ];
    const after = await this.repository.update<SuccessStoryRecord>(COLLECTIONS.successStories, id, sanitizePayload({ featured, timeline }));
    await this.log(request, featured ? "FEATURE_SUCCESSSTORY" : "UNFEATURE_SUCCESSSTORY", "successStories", id, before, after);
    return this.enrichSuccessStory(after);
  }

  async generateSuccessStoryDraft(dto: Record<string, unknown>, request: AuthenticatedRequest) {
    const fallback = this.heuristicSuccessStoryDraft(dto);
    const apiKey = this.config.get<string>("OPENAI_API_KEY");
    if (!apiKey) return fallback;

    try {
      const model = this.config.get<string>("OPENAI_SUCCESS_STORY_MODEL")
        ?? this.config.get<string>("OPENAI_REPAIR_TRIAGE_MODEL")
        ?? "gpt-5-mini";
  ...
  async listSupportOperations() {
    const settled = await Promise.allSettled([
      this.repository.list<SupportTicketRecord>(COLLECTIONS.supportTickets, 500),
      ...
      this.repository.list<Record<string, unknown> & { id: string }>(COLLECTIONS.deployments, 500)
    ]);
    const value = <T>(index: number, fallback: T): T =>
      settled[index].status === "fulfilled" ? settled[index].value as T : fallback;
    const tickets = value<SupportTicketRecord[]>(0, []).map((ticket) => this.enrichSupportTicket(ticket));
    return {
      tickets,
  ...
  async getSupportTicket(id: string) {
    return this.enrichSupportTicket(await this.repository.findById<SupportTicketRecord>(COLLECTIONS.supportTickets, id));
  }

  async createSupportTicketRecord(dto: Record<string, unknown>, request: AuthenticatedRequest) {
    ...
    }, request) as SupportTicketRecord);
  }

  async updateSupportTicketRecord(id: string, dto: Record<string, unknown>, request: AuthenticatedRequest) {
    const before = await this.repository.findById<SupportTicketRecord>(COLLECTIONS.supportTickets, id);
    const timeline = [
      ...this.asArray(before.timeline),
    ...
    const assignedTo = this.safeString(dto.assignedTo ?? dto.assignedOwner ?? dto.owner);
    const after = await this.repository.update<SupportTicketRecord>(COLLECTIONS.supportTickets, id, sanitizePayload({
      ...dto,
  ...
  async assignSupportTicket(id: string, dto: Record<string, unknown>, request: AuthenticatedRequest) {
    const assignedTo = this.safeString(dto.assignedTo ?? dto.assignedOwner ?? dto.owner);
    if (!assignedTo) throw new BadRequestException("An assignee is required.");
  ...
  async escalateSupportTicket(id: string, dto: Record<string, unknown>, request: AuthenticatedRequest) {
    return this.updateSupportTicketRecord(id, {
      ...dto,
      status: "ESCALATED",
  ...
  async closeSupportTicket(id: string, dto: Record<string, unknown>, request: AuthenticatedRequest) {
    return this.updateSupportTicketRecord(id, {
      ...dto,
      status: "CLOSED",
  ...
  async linkSupportRecord(id: string, dto: Record<string, unknown>, request: AuthenticatedRequest) {
    ...
    if (!recordType || !recordId) throw new BadRequestException("A record type and record id are required.");
    ...
    if (!key) throw new BadRequestException("Unsupported linked record type.");
    return this.updateSupportTicketRecord(id, {
      [key]: recordId,
  ...
  async listTrainingCohortOperations() {
    const cohorts = (await this.safeList<TrainingCohortRecord>(COLLECTIONS.trainingCohorts, 500))
  ...
  async getTrainingCohort(id: string) {
    return this.enrichTrainingCohort(await this.repository.findById<TrainingCohortRecord>(COLLECTIONS.trainingCohorts, id));
  }

  async createTrainingCohortRecord(dto: Record<string, unknown>, request: AuthenticatedRequest) {
    ...
    }, request) as TrainingCohortRecord);
  }

  async updateTrainingCohortRecord(id: string, dto: Record<string, unknown>, request: AuthenticatedRequest) {
    const before = await this.repository.findById<TrainingCohortRecord>(COLLECTIONS.trainingCohorts, id);
    ...
    const after = await this.repository.update<TrainingCohortRecord>(COLLECTIONS.trainingCohorts, id, sanitizePayload({
      ...dto,
  ...
  async importTrainingLearners(id: string, file: UploadedAdminFile | undefined, request: AuthenticatedRequest) {
    if (!file?.buffer?.length) {
      throw new BadRequestException("A learner CSV file is required.");
    }
    const before = await this.repository.findById<TrainingCohortRecord>(COLLECTIONS.trainingCohorts, id);
    const imported = this.parseTrainingLearnerCsv(file.buffer.toString("utf8"));
    if (!imported.length) {
      throw new BadRequestException("No learners were found in the CSV file.");
    }
  ...
  async generateTrainingCertificates(id: string, request: AuthenticatedRequest) {
    const before = await this.repository.findById<TrainingCohortRecord>(COLLECTIONS.trainingCohorts, id);
    const learners = this.trainingLearners(this.asArray(before.learnerRegister));
    if (!learners.length) {
      throw new BadRequestException("Learner register is required before certificates can be generated.");
    }
    const bucket = this.firebaseAdmin.storage.bucket();
    const stamp = Date.now();
    ...
    const generatedAt = new Date().toISOString();
    const artifact = sanitizePayload({
      pdf: {
  ...
  async markTrainingCohortActive(id: string, request: AuthenticatedRequest) {
    return this.updateTrainingCohortRecord(id, { status: "ACTIVE" }, request);
  }

  async completeTrainingCohort(id: string, request: AuthenticatedRequest) {
    return this.updateTrainingCohortRecord(id, { status: "COMPLETED", completionRate: 100 }, request);
  }

  async exportTrainingRegister(id: string) {
    const cohort = this.enrichTrainingCohort(await this.repository.findById<TrainingCohortRecord>(COLLECTIONS.trainingCohorts, id));
  ...
  async listSustainabilityReportOperations() {
    const [reports, source] = await Promise.all([
      this.safeList<SustainabilityReportRecord>(COLLECTIONS.sustainabilityReports, 500),
  ...
  async getSustainabilityReport(id: string) {
    const report = await this.repository.findById<SustainabilityReportRecord>(COLLECTIONS.sustainabilityReports, id);
  ...
  async generateSustainabilityReport(dto: Record<string, unknown>, request: AuthenticatedRequest) {
    ...
    }, request) as SustainabilityReportRecord;

    const artifacts = await this.generateSustainabilityArtifacts(created.id, this.enrichSustainabilityReport(created));
    const updated = await this.repository.update<SustainabilityReportRecord>(COLLECTIONS.sustainabilityReports, created.id, sanitizePayload({
      artifacts,
      timeline: [
  ...
  async exportSustainabilityReport(id: string, format: "pdf" | "csv") {
  ...
  async seedDefaultStories(request: AuthenticatedRequest) {
    const existing = await this.repository.list<SuccessStoryRecord>(COLLECTIONS.successStories, 5);
    if (existing.length) return existing.map((story) => this.enrichSuccessStory(story));
  ...
  private async sustainabilitySourceSnapshot() {
    const settled = await Promise.allSettled([
      this.repository.list<Record<string, unknown> & { id: string }>(COLLECTIONS.inventory, 500),
      ...
      this.repository.list<Record<string, unknown> & { id: string }>(COLLECTIONS.repairTickets, 500),
      this.repository.list<Record<string, unknown> & { id: string }>(COLLECTIONS.deployments, 500),
      this.repository.list<Record<string, unknown> & { id: string }>(COLLECTIONS.successStories, 200),
      this.repository.getSingleton<Record<string, unknown>>(COLLECTIONS.impactStats, "current", {})
    ]);
    const value = <T>(index: number, fallback: T): T =>
      settled[index].status === "fulfilled" ? settled[index].value as T : fallback;
    const inventory = value<Array<Record<string, unknown> & { id: string }>>(0, []);
    const donations = value<Array<Record<string, unknown> & { id: string }>>(1, []);
    const recycling = value<Array<Record<string, unknown> & { id: string }>>(2, []);
    const repairs = value<Array<Record<string, unknown> & { id: string }>>(3, []);
    const deployments = value<Array<Record<string, unknown> & { id: string }>>(4, []);
    const successStories = value<Array<Record<string, unknown> & { id: string }>>(5, []);
    const impact = value<Record<string, unknown>>(6, {});
    ...
    const co2EstimatedKg = Math.max(
      this.safeNumber(impact.co2SavedKg) ?? 0,
      devicesReused * 75 + devicesRecycled * 50 + completedRepairs * 45 + recycling.reduce((sum, item) => sum + (this.safeNumber(item.estimatedCo2KgAvoided ?? item.estimatedCo2SavedKg) ?? 0), 0)
    );
    const totalRouted = Math.max(1, devicesReused + devicesRecycled);
    const evidenceSignals =
      recycling.filter((item) => this.asArray(item.attachments).length || this.asArray(item.reportPacks).length || item.sustainabilityReportId).length +
  ...
  private enrichSustainabilityReport(report: SustainabilityReportRecord): SustainabilityReportRecord {
    ...
    }) as SustainabilityReportRecord;
  }

  private sustainabilityReportStatus(value: unknown) {
  ...
  private sustainabilityReportType(value: unknown) {
  ...
  private sustainabilityReportTitle(type: string, periodStart: string, periodEnd: string) {
  ...
  private async generateSustainabilityArtifacts(reportId: string, report: SustainabilityReportRecord) {
    const bucket = this.firebaseAdmin.storage.bucket();
    const stamp = Date.now();
  ...
  private sustainabilityReportCsv(report: SustainabilityReportRecord) {
  ...
  private async sustainabilityReportPdf(report: SustainabilityReportRecord) {
    ...
    page.drawRectangle({ x: 0, y: 760, width: 595, height: 82, color: rgb(0.08, 0.08, 0.08) });
    page.drawText("SIT Digital Access", { x: 48, y: 802, size: 14, font: bold, color: rgb(1, 1, 1) });
    page.drawText("Sustainability & ESG Reporting Pack", { x: 48, y: 778, size: 20, font: bold, color: rgb(1, 1, 1) });
    page.drawText(new Date().toLocaleDateString("en-GB"), { x: 460, y: 806, size: 9, font: regular, color: rgb(1, 0.82, 0.68) });
    page.drawText(title.slice(0, 82), { x: 48, y: 724, size: 16, font: bold, color: rgb(0.08, 0.08, 0.08) });

    let y = 690;
    for (const [label, value] of rows) {
      page.drawText(label, { x: 48, y, size: 10, font: bold, color: rgb(0.24, 0.24, 0.24) });
      page.drawText(value.slice(0, 76), { x: 230, y, size: 10, font: regular, color: rgb(0.1, 0.1, 0.1) });
      y -= 26;
    }
  ...
  private recyclingSummary(
    ...
    const processingStatuses: RecyclingStatus[] = [
      "SECURE_WIPE_PENDING",
      "SECURE_WIPE_COMPLETE",
      "ASSESSMENT",
      "REFURBISH_APPROVED",
  ...
  private recyclingStatus(value: unknown): RecyclingStatus {
    ...
    return recyclingStatuses.includes(candidate as RecyclingStatus) ? candidate as RecyclingStatus : "INTAKE_CREATED";
  }

  private recyclingStatusLabel(status: RecyclingStatus) {
    return status
      .toLowerCase()
      .split("_")
  ...
  private recyclingCollectionDueAt(record: RecyclingRecord) {
  ...
  private defaultSecureWipeChecklist(dto: Record<string, unknown>) {
  ...
  private async generateRecyclingRecommendation(record: RecyclingRecord) {
    const fallback = this.heuristicRecyclingRecommendation(record);
    const apiKey = this.config.get<string>("OPENAI_API_KEY");
    if (!apiKey) return fallback;

    try {
      const model = this.config.get<string>("OPENAI_RECYCLING_RECOMMENDATION_MODEL")
        ?? this.config.get<string>("OPENAI_REPAIR_TRIAGE_MODEL")
        ?? "gpt-5-mini";
      const response = await fetch("https://api.openai.com/v1/responses", {
        method: "POST",
  ...
  private heuristicRecyclingRecommendation(record: RecyclingRecord) {
    ...
    if (dataBearingCount > 0 || record.secureWipeRequired === true) riskFlags.push("Secure wipe required");
    if (record.chainOfCustodyRequired === true) riskFlags.push("Chain of custody required");
    if (/broken|scrap|e[- ]?waste|liquid|damaged|parts/.test(text)) riskFlags.push("Likely end-of-life assets");
  ...
  private recyclingReportCsv(record: RecyclingRecord) {
    const rows: Array<[string, unknown]> = [
  ...
  private async recyclingReportPdf(record: RecyclingRecord) {
    const pdf = await PDFDocument.create();
    const page = pdf.addPage([595, 842]);
    const regular = await pdf.embedFont(StandardFonts.Helvetica);
    const bold = await pdf.embedFont(StandardFonts.HelveticaBold);
    ...
    page.drawRectangle({ x: 0, y: 760, width: 595, height: 82, color: rgb(0.08, 0.08, 0.08) });
    page.drawText("SIT Digital Access", { x: 48, y: 802, size: 14, font: bold, color: rgb(1, 1, 1) });
    page.drawText("Recycling & Circular Technology ESG Pack", { x: 48, y: 778, size: 20, font: bold, color: rgb(1, 1, 1) });
    page.drawText(`Generated ${new Date().toLocaleDateString("en-GB")}`, { x: 420, y: 806, size: 9, font: regular, color: rgb(1, 0.82, 0.68) });

    page.drawText("Donor evidence summary", { x: 48, y: 724, size: 16, font: bold, color: rgb(0.08, 0.08, 0.08) });
    let y = 690;
    for (const [label, value] of rows) {
      page.drawText(label, { x: 48, y, size: 10, font: bold, color: rgb(0.24, 0.24, 0.24) });
      page.drawText(value.slice(0, 72), { x: 230, y, size: 10, font: regular, color: rgb(0.1, 0.1, 0.1) });
      y -= 26;
    }
  ...
  private enrichSuccessStory(story: SuccessStoryRecord) {
  ...
  private successStorySummary(stories: SuccessStoryRecord[]) {
    const regions = new Set(
      stories
        .map((story) => this.safeString(story.region ?? story.country))
        .filter((value): value is string => Boolean(value))
    );
    const status = (story: SuccessStoryRecord) => this.successStoryStatus(story.status, story.published);
    return {
      ...
      impactMetricsAttached: stories.filter((story) =>
        this.asArray(story.metrics).length > 0 ||
        (this.safeNumber(story.devicesProvided ?? story.deviceCount) ?? 0) > 0 ||
        Boolean(this.safeString(story.outcome ?? story.trainingLinked))
      ).length
    };
  }

  private successStoryStatus(value: unknown, published?: unknown) {
    const status = String(value ?? "").trim().toUpperCase().replace(/[\s-]+/g, "_");
    if (status === "PUBLISHED" || status === "LIVE" || published === true) return "PUBLISHED";
  ...
  private successStoryType(value: unknown) {
  ...
  private successStoryTypeLabel(value: string) {
  ...
  private storyMediaUrls(story: Record<string, unknown>) {
    const urls = this.asArray(story.mediaUrls)
      .map(String)
      .filter((value) => value.trim().length > 0);
    const visualAsset = this.safeString(story.visualAsset);
    if (visualAsset && !urls.includes(visualAsset)) urls.unshift(visualAsset);
    return urls;
  }

  private storySlug(value: string) {
    const slug = value
      .toLowerCase()
      .replace(/['"]/g, "")
      .replace(/[^a-z0-9]+/g, "-")
      .replace(/^-+|-+$/g, "")
      .slice(0, 80);
    return slug || `story-${randomBytes(3).toString("hex")}`;
  }

  private successStoryTimelineForUpdate(
    before: SuccessStoryRecord,
    dto: Record<string, unknown>,
    request: AuthenticatedRequest
  ) {
    const entries: Array<Record<string, unknown>> = [];
  ...
  private heuristicSuccessStoryDraft(dto: Record<string, unknown>) {
  ...
  private enrichSupportTicket(ticket: SupportTicketRecord) {
  ...
  private supportSummary(
    ...
    _inventory: Array<Record<string, unknown>>,
    _repairs: Array<Record<string, unknown>>,
    _deployments: Array<Record<string, unknown>>
  ) {
  ...
  private supportStatus(value: unknown): SupportStatus {
    const status = String(value ?? "NEW").toUpperCase().replace(/[\s-]+/g, "_");
    const mapped: Record<string, SupportStatus> = {
    ...
    return supportStatuses.includes(candidate as SupportStatus) ? candidate as SupportStatus : "NEW";
  }

  private supportPriority(value: unknown): SupportPriority {
    const priority = String(value ?? "MEDIUM").toUpperCase().replace(/[\s-]+/g, "_");
    return supportPriorities.includes(priority as SupportPriority) ? priority as SupportPriority : "MEDIUM";
  }

  private supportCategory(value: unknown): SupportCategory {
    const category = String(value ?? "GENERAL_ENQUIRY").toUpperCase().replace(/[\s-/]+/g, "_");
    const mapped: Record<string, SupportCategory> = {
    ...
    return supportCategories.includes(candidate as SupportCategory) ? candidate as SupportCategory : "GENERAL_ENQUIRY";
  }
  ...
  private supportSlaDueAt(source: Record<string, unknown>) {
  ...
  private supportSlaHours(value: unknown) {
  ...
  private supportDueAt(ticket: SupportTicketRecord) {
  ...
  private supportNoteLog(existing: unknown[], note: unknown, request: AuthenticatedRequest) {
  ...
  private enrichTrainingCohort(cohort: TrainingCohortRecord) {
  ...
  private trainingCohortSummary(cohorts: TrainingCohortRecord[]) {
    const totalLearners = cohorts.reduce((sum, cohort) =>
      sum + (this.safeNumber(cohort.enrolledLearners ?? cohort.learnerCount) ?? this.asArray(cohort.learnerRegister).length), 0);
    const average = (key: string) => {
      const values = cohorts.map((cohort) => this.safeNumber(cohort[key])).filter((value): value is number => value !== null);
    ...
    return {
      ...
      certificationReady: cohorts.filter((cohort) =>
        this.trainingStatus(cohort.status) === "CERTIFICATION_READY" ||
        (this.safeNumber(cohort.certificationReadiness) ?? 0) >= 80
      ).length,
      ...
      attendanceRisk: cohorts.filter((cohort) =>
        this.trainingStatus(cohort.status) === "AT_RISK" ||
        (this.safeNumber(cohort.attendanceRate) ?? 100) < 70
      ).length
    };
  }

  private trainingStatus(value: unknown): TrainingStatus {
    const status = String(value ?? "DRAFT").toUpperCase().replace(/[\s-]+/g, "_");
    const mapped: Record<string, TrainingStatus> = {
      PLANNING: "DRAFT",
      PLANNED: "DRAFT",
      OPEN: "RECRUITING",
      RUNNING: "ACTIVE",
    ...
    const candidate = mapped[status] ?? status;
    return trainingStatuses.includes(candidate as TrainingStatus) ? candidate as TrainingStatus : "DRAFT";
  }

  private trainingProgrammeType(value: unknown): TrainingProgrammeType {
    const type = String(value ?? "DIGITAL_LITERACY").toUpperCase().replace(/[\s-/]+/g, "_");
    const mapped: Record<string, TrainingProgrammeType> = {
      AI: "AI_LITERACY",
    ...
    const candidate = mapped[type] ?? type;
    return trainingProgrammeTypes.includes(candidate as TrainingProgrammeType) ? candidate as TrainingProgrammeType : "DIGITAL_LITERACY";
  }

  private trainingProgrammeTypeLabel(value: TrainingProgrammeType) {
    const labels: Record<TrainingProgrammeType, string> = {
  ...
  private trainingDeliveryMode(value: unknown) {
    const mode = String(value ?? "IN_PERSON").toUpperCase().replace(/[\s-]+/g, "_");
    if (mode === "IN_PERSON" || mode === "ONLINE" || mode === "HYBRID") return mode;
    return "IN_PERSON";
  }

  private trainingLearners(values: unknown[]) {
    return values
      .map((value, index) => {
        ...
        if (!name) return null;
        return sanitizePayload({
          id: this.safeString(item.id) ?? randomBytes(6).toString("hex"),
          name,
  ...
  private averageLearnerMetric(learners: Array<Record<string, unknown>>, key: string) {
    const values = learners.map((learner) => this.safeNumber(learner[key])).filter((value): value is number => value !== null);
    return values.length ? Math.round(values.reduce((sum, value) => sum + value, 0) / values.length) : 0;
  }

  private trainingCertificationReadiness(source: Record<string, unknown>) {
  ...
  private parseTrainingLearnerCsv(csv: string) {
    const rows = this.parseCsv(csv);
    if (rows.length <= 1) return [];
    const headers = rows[0].map((header) => header.trim().toLowerCase().replace(/[^a-z0-9]+/g, ""));
    const index = (names: string[]) => names.map((name) => headers.indexOf(name)).find((item) => item >= 0) ?? -1;
  ...
  private async trainingCertificatePdf(cohort: TrainingCohortRecord, learners: Array<Record<string, unknown>>) {
    const pdf = await PDFDocument.create();
    const regular = await pdf.embedFont(StandardFonts.Helvetica);
    const bold = await pdf.embedFont(StandardFonts.HelveticaBold);
    ...
    for (const learner of learners) {
      ...
      page.drawRectangle({ x: 0, y: 515, width: 842, height: 80, color: rgb(0.08, 0.08, 0.08) });
      page.drawText("SIT Digital Access", { x: 58, y: 555, size: 14, font: bold, color: rgb(1, 1, 1) });
      page.drawText("Certificate of Completion", { x: 58, y: 525, size: 28, font: bold, color: rgb(1, 0.82, 0.68) });
      page.drawText("This certifies that", { x: 58, y: 430, size: 16, font: regular, color: rgb(0.25, 0.25, 0.25) });
      page.drawText(String(learner.name ?? "Learner").slice(0, 64), { x: 58, y: 385, size: 30, font: bold, color: rgb(0.08, 0.08, 0.08) });
      page.drawText(`completed ${programme} as part of ${cohortName}.`, { x: 58, y: 340, size: 15, font: regular, color: rgb(0.22, 0.22, 0.22), maxWidth: 720 });
      page.drawText(`Attendance: ${learner.attendanceRate ?? cohort.attendanceRate ?? 0}%`, { x: 58, y: 285, size: 12, font: regular, color: rgb(0.3, 0.3, 0.3) });
      page.drawText(`Completion: ${learner.completionRate ?? cohort.completionRate ?? 0}%`, { x: 58, y: 260, size: 12, font: regular, color: rgb(0.3, 0.3, 0.3) });
      page.drawText(`Issued ${new Date().toLocaleDateString("en-GB")}`, { x: 58, y: 150, size: 11, font: regular, color: rgb(0.35, 0.35, 0.35) });
      page.drawText("Training Operations & Certification Centre", { x: 58, y: 118, size: 11, font: bold, color: rgb(0.08, 0.08, 0.08) });
      page.drawRectangle({ x: 58, y: 82, width: 210, height: 2, color: rgb(0.95, 0.42, 0.13) });
    }
    return pdf.save();
  }
